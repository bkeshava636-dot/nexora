...
// For Semester QPs
code = code.replace(
  /<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">[\s\S]*?<button[\s\S]*?Add Semester QP[\s\S]*?<\/button>\s*<\/div>/,
...
// We need to move the hooks to the top of the function
code = code.replace(/const \[deptManagerOpen, setDeptManagerOpen\] = useState\(false\);[\s\S]*?updateSemQpDept = .*?\n/, '');
code = code.replace(/function AdminSemesterQpsSection\(\) \{/, `function AdminSemesterQpsSection() {\n  const [deptManagerOpen, setDeptManagerOpen] = useState(false);\n  const { data: semQpDepts = [] } = useListSemesterQpDepartments({ includeInactive: true });\n  const createSemQpDept = useCreateSemesterQpDepartment();\n  const updateSemQpDept = useUpdateSemesterQpDepartment();`);


// For IA Papers
code = code.replace(
  /<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">\s*<div>\s*<h2 className="text-xl font-bold text-\[hsl\(var\(--foreground\)\)\]">Internal Assessment Papers<\/h2>[\s\S]*?<button[\s\S]*?Add IA Paper[\s\S]*?<\/button>\s*<\/div>/,
  `const [deptManagerOpen, setDeptManagerOpen] = useState(false);
...
code = code.replace(/const \[deptManagerOpen, setDeptManagerOpen\] = useState\(false\);[\s\S]*?updateIaDept = .*?\n/, '');
