...
export const getListAdminQuickLinksQueryKey = (params?: ListQuickLinksParams) =>
  params ? ["/api/admin/quick-links", params] : ["/api/admin/quick-links"];

export function useListAdminQuickLinks(
  params?: ListQuickLinksParams,
  options?: Omit<UseQueryOptions<ImportantLinkItem[], Error>, "queryKey" | "queryFn">,
) {
  return useQuery<ImportantLinkItem[], Error>({
    queryKey: getListAdminQuickLinksQueryKey(params),
    queryFn: () => {
      const sp = new URLSearchParams();
      if (params?.category) sp.set("category", params.category);
      if (params?.search) sp.set("search", params.search);
      if (params?.isActive) sp.set("isActive", params.isActive);
      const q = sp.toString();
      return customFetch("/api/admin/quick-links" + (q ? "?" + q : ""));
    },
    ...options,
  });
}
...
export function useCreateQuickLink() {
  ...
  return useMutation<ImportantLinkItem, Error, { data: CreateQuickLinkInput }>({
    mutationFn: ({ data }) =>
      customFetch("/api/admin/quick-links", {
        method: "POST",
...
export function useUpdateQuickLink() {
  ...
  return useMutation<ImportantLinkItem, Error, { id: number; data: UpdateQuickLinkInput }>({
    mutationFn: ({ id, data }) =>
      customFetch("/api/admin/quick-links/" + id, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      }),
    onSuccess: (_, { id }) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/admin/quick-links", id] });
    },
  });
}
...
export function useToggleQuickLinkStatus() {
  ...
  return useMutation<ImportantLinkItem, Error, { id: number; isActive: boolean }>({
    mutationFn: ({ id, isActive }) =>
      customFetch("/api/admin/quick-links/" + id + "/status", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
